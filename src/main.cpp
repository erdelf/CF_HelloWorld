...
namespace HelloWorld
{
    ...
    struct BasicObject
    {
        ...
        std::unique_ptr<void> collisionShape = nullptr;
        ...
        {
            //Hardcoded for circles atm. Best case would be to check all different collision shapes
            const float combinedShapeDistance = ((CF_Circle*)collisionShape.get())->r + ((CF_Circle*)other->collisionShape.get())->r;
    ...
    struct BouncingCircle final : BasicObject
    {
        ...
        BouncingCircle()
        {
            ...
            collisionShape = std::make_unique<CF_Circle>(CF_Circle{ position, highestRadius });
        }
    ...
    static void fixed_update(void* udata)
    {
        ...
        for (const auto& objA : objects)
            for (const auto& objB : objects)
                objA->CollisionTestWith(objB.get());
    }
...
{
    if (const CF_Result result = cf_make_app("MainWindow", 0, 0, 0, HelloWorld::width, HelloWorld::height, CF_APP_OPTIONS_WINDOW_POS_CENTERED_BIT, argv[0]); cf_is_error(result))
        return -1;
    ...
    {
        cf_app_update(HelloWorld::fixed_update);
